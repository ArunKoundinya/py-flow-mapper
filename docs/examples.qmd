...
The Mermaid below is an example of the detailed flow output you’ll get. Solid arrows show function calls. Dashed arrows represent return-value data flow (e.g., a value assigned to a variable and later passed along).
...
```bash
pyflow analyze ./tests/ --entry-point tests/main_new.py
```

This creates 

- `./tests/project_meta.json`.

### Generate diagrams

```bash
pyflow diagram ./tests/project_meta.json
```
...
graph LR
    ...
    subgraph External [External]
        ext_MermaidGenerator[MermaidGenerator]
        ext_ProjectAnalyzer[ProjectAnalyzer]
        Done((Done))
    end
    main_new_test_flow_analysis --> main_new_create_flow_test_project
    main_new_test_flow_analysis --> ext_ProjectAnalyzer
    main_new_test_flow_analysis --> |meta_file| ext_MermaidGenerator
    main_new_main --> main_new_test_flow_analysis
    main_new_create_flow_test_project -.->|project_dir| main_new_test_flow_analysis
    ext_ProjectAnalyzer -.->|analyzer| main_new_test_flow_analysis
    ext_MermaidGenerator -.->|generator| main_new_test_flow_analysis
    ext_ProjectAnalyzer -->|meta_file| ext_MermaidGenerator
    ext_MermaidGenerator -->|output| Done
```

## Example-3 - Analyze PyFlowMapper itself (self-analysis)
...
```bash
pyflow analyze ./src/py-flow-mapper/ --entry-point cli.py
```
This creates 
